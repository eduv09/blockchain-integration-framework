...
export async function getIntegrationsService(
  ...
  const supportedSystems = manager.ReachableDLTs;

  return supportedSystems.map((supportedSystem) =>
    convertReachableDLTsIntoIntegrations(supportedSystem),
  );
}

function convertReachableDLTsIntoIntegrations(
  networkId: NetworkId,
): Integration {
  switch (networkId.ledgerType) {
    ...
    case LedgerType.Ethereum:
      return {
        id: networkId.id,
        name: "Ethereum", //consider change NetworkId to Integration-like object
